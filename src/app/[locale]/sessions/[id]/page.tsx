...
export default async function SessionDetailPage({
  ...
  // auto-generated session の場合、description から tendency ID を取得して元情報を取り出す
  const tendencyMatch = session.description?.match(/\[tendency:([^\]]+)\]/);
  const tendencyId = tendencyMatch?.[1] ?? null;
  const matchedTendency = tendencyId
    ? (session.venue?.tendencies ?? []).find((ten) => ten.id === tendencyId)
    : null;
  // description から [tendency:...] 行を取り除いてクリーンにする
  const cleanDescription = session.description
    ?.split('\n')
    .filter((line) => !line.includes('[tendency:'))
    .join('\n')
    .trim() ?? null;
  ...
  return (
    ...
    <div className="max-w-3xl space-y-8">
      ...
      {/* Auto-generated notice + source URL */}
      {session.description?.includes('[tendency:') && session.venue && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm space-y-2">
          <p className="text-amber-800 font-medium">{t('session.recurringSession')}</p>
          <p className="text-amber-700 text-xs">{t('session.autoCollectedNotice')}</p>
          <div className="flex flex-wrap gap-3 pt-1">
            {matchedTendency?.sourceUrl && (
              <a
                href={matchedTendency.sourceUrl}
                target="_blank"
                rel="noopener noreferrer"
              ...
              >
                {t('session.verifySource')}
              </a>
            )}
            <Link
              href={`/${locale}/venues/${session.venue.id}`}
              className="inline-flex items-center gap-1.5 rounded-md border border-amber-300 px-3 py-1.5 text-xs text-amber-700 hover:bg-amber-100 transition-colors"
            >
              {t('session.checkVenuePage')}
            </Link>
          </div>
          {matchedTendency?.entrySystem && (
            <p className="text-xs text-amber-700">
              🎫 {t('session.entryFee')}: {matchedTendency.entrySystem}
            </p>
          )}
        </div>
      )}
